//! Sequential Kazhdan–Lusztig polynomial computation, **equal parameters**.
//!
//! This is the heart of the port.  Reference:
//! `pycox-ref/pycox_ref.py::klpolynomials` (≈10141–10470).  The PyCox source
//! is normative; this module is a faithful translation of its `uneq == False`
//! path (all weights `1`).  Task 10 lifts the equal-parameter restriction.
//!
//! # Algorithm overview
//!
...
//! Case II) always reference an already-computed higher index, and what makes
//! the polynomial pool grow in PyCox's exact insertion order.
//!
//! Each `(w, y)` entry gets three things, in order:
//!
//! 1. **Bruhat flag** ([`bruhat_flag`]) — comparable (`c`) or not (`f`).
//!    Fused into the row loop; PyCox computes the whole flag matrix first, but
//!    every flag this row reads (`mat[aw0[y]][aw0[w]]`, `mat[sw][..]`) lives in
//!    a strictly shorter — hence already complete — row.
//! 2. **P̃_{y,w}** ([`compute_h`]) — the KL polynomial.
//! 3. **mu presence flags** — set iff `lft(y,s) < y && lft(w,s) > w`.  In
//!    Implicit mode the value is derived on demand from the polynomial.
//!
...
//! `lweights`, the **completed** rows, and the **frozen** global polynomial
//! pool) and returns a [`RowResult`]: the per-`y` polynomial *values* of this
//! row plus the presence flags.  The kernel never touches the global pool for
...
//! reproduce the sequential pool exactly.
//!
...
use crate::{
    ...
    group::CoxeterGroup,
    kl::{
        table::{KlRow, KlTable, MuMode, NOT_LEQ},
        KlError, KlOpts,
    },
    laurent::Laurent,
};
...
/// **completed** rows, and the **frozen** global polynomial pool that the
/// kernel reads from.  All reads target strictly-shorter rows (full
/// recursion, `aw0` symmetry, Cases I/II shorter-row reads) or the inverse
/// row (inverse symmetry, same length — completed earlier in the ascending-`w`
/// driver loop).  Same-row reads do **not** use this context; they read the
/// in-progress row's own computed values.
...
/// The output of [`compute_row`]: per-`y` polynomial values and mu presence
/// flags, not yet interned into the global pool.
pub(crate) struct RowResult {
    /// `pol[y]` is the computed `P̃_{y,w}` (or `Incomparable`), `y` in `0..=w`.
    pub pol: Vec<PolSlot>,
    /// Flat `(w+1) * rank` mu-slot presence flags.
    pub mu_present: Vec<bool>,
}
...
/// Compute one row `w` of the KL table (equal parameters) as pure data.
///
/// Returns the per-`y` polynomial values and mu presence flags.  The caller
/// (sequential or parallel driver) interns the values into the global pool.
pub(crate) fn compute_row(w: ElmIdx, ctx: &KlCtx<'_>) -> RowResult {
    ...
    let mut mu_present = vec![false; (w_us + 1) * rank];
    ...
    for y in (0..w).rev() {
        ...
        // ---- Phase B: P̃_{y,w} ----
        let h = compute_h(w, y, first_desc, ctx, &pol);
        pol[y as usize] = PolSlot::Value(h);

        // ---- Phase C: mu presence flags (equal parameters) ----
        for s in 0..rank {
            if elms.lft(y, s) < y && elms.lft(w, s) > w {
                mu_present[y as usize * rank + s] = true;
            }
        }
    }

    RowResult { pol, mu_present }
}
...
/// Compute the full KL table sequentially for **equal parameters**.
///
/// Validates `opts` against `group`, then asserts all weights are `1` (Task 10
/// lifts this via [`KlError::Unimplemented`]).  Builds the element table and an
/// empty Implicit-mode table, seeds row `0`, and fills rows `1..n` via
/// [`compute_row`], interning each row's values in `(w asc, y desc)` order so
/// the pool grows in PyCox's exact insertion order.
pub fn klpolynomials_seq(group: &CoxeterGroup, opts: &KlOpts) -> Result<KlTable, KlError> {
    opts.validate(group)?;
    if opts.weights.iter().any(|&wt| wt != 1) {
        return Err(KlError::Unimplemented("unequal parameters: Task 10"));
    }

    let elms = ElementTable::build(group);
    let n = elms.len();
    let n_pos = elms.lengths.iter().copied().max().unwrap_or(0);
    let mut table = KlTable::new_empty(elms, opts.weights.clone(), MuMode::Implicit);
    let rank = table.elms.rank;

    // Dedup map beside the pool.  Seeded with pols[0] = one at id 0.
    ...
    for w in 1..n {
        // Build the read-only context over the completed rows + frozen pool.
        let ctx = KlCtx {
            elms: &table.elms,
            n_pos,
        ...
        let row = intern_row(&mut table.pols, &mut pool_index, result);
        table.rows.push(row);
    }
...
/// Intern a [`RowResult`] into the global pool in descending-`y` order,
...
fn intern_row(
    ...
    // Descending y so pool insertion order matches PyCox exactly.
    for (y, slot) in result.pol.into_iter().enumerate().rev() {
        match slot {
            PolSlot::Incomparable => {} // leave NOT_LEQ
            PolSlot::Value(p) => {
                pol_ids[y] = intern(pols, pool_index, p);
            }
        }
    }
    KlRow {
        pol: pol_ids,
        mu: None,
        mu_present: Some(result.mu_present),
    }
}

/// Intern `p` into `pols`, deduplicating via `pool_index`; return its id.
fn intern(pols: &mut Vec<Laurent>, pool_index: &mut HashMap<Laurent, u32>, p: Laurent) -> u32 {
...
mod tests {
    ...
    /// Unequal parameters return the Task-10 Unimplemented error.
    #[test]
    fn unequal_is_unimplemented() {
        ...
        let r = klpolynomials_seq(&g, &opts);
        assert!(matches!(r, Err(KlError::Unimplemented(_))));
    }
